...
Чистая Архитектура для Flutter от Reso Coder

        ↗ Presentation              На схеме мы имеем три  слоя, объединенный в feature 
feature → Domain
        ↘ Data
...
Очевидно, что этот слой будет содержать виджеты  и состояния. Для управления состоянием мы будем использовать Bloc. 
...
Слой Domain это центр приложения, независящий от других слоев. Но другие слои зависят от Domain. Этот уровень содержит основную бизнес-логику (Use Cases) и бизнес-объекты(Entities). После того как получает данные из слоя данных, становится абсолютно независимым. "Repositories" относится одновременно к слоям Domain и Data, И чтобы реализовать передачу данных между слоями, будем пользоваться Принципом инверсии зависимостей, используя Interface(abstract class). Таким образом в слое Domain мы будем создавать абстрактный класс репозитория, а на уровне данных будем его реализовывать. Так же на слое Domain мы будем обрабатывать ошибки. 
...
Слой данных состоит из реализации репозитория, а так же источники данных: один для получения данных, другой для кэширования. Информацию о персонажах мы будем получать в формате json, поэтому источники данных возвращают не сущности данных (Entities) а модели (Models). Это необходимо из-за того, что нам необходимо преобразовать сырые данные из json - Сериализовать json, для того чтобы специфичный для json код не попал в ядро программы. Поэтому мы будем создать Models и расширять Entities, добавляя методы toJson и fromJson. 
